...
export class PaymentService {
  ...
  async createPurchase(paymentData: CreatePaymentDto): Promise<PaymentResponse> {
    return this.executeWithIdempotency(
      ...
      async () => {
        ...
        try {
          ...
          const result = await this.authorizeNetService.createPurchaseTransaction(paymentData);

          // Update transaction with result
          savedTransaction.status = result.success ? TransactionStatus.COMPLETED : TransactionStatus.FAILED;
          ...
          this.logger.log(`Purchase transaction ${savedTransaction.id} ${result.success ? 'completed' : 'failed'}`);
  ...
  async createAuthorization(paymentData: CreatePaymentDto): Promise<PaymentResponse> {
    return this.executeWithIdempotency(
      paymentData.idempotencyKey,
      async () => {
        const queryRunner = this.dataSource.createQueryRunner();
        ...
        try {
          ...
          const result = await this.authorizeNetService.createAuthorizationTransaction(paymentData);
          ...
          this.logger.log(`Authorization transaction ${savedTransaction.id} ${result.success ? 'completed' : 'failed'}`);
  ...
  async capturePayment(captureData: CapturePaymentDto): Promise<PaymentResponse> {
    return this.executeWithIdempotency(
      captureData.idempotencyKey,
      async () => {
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        ...
        try {
          ...
          const originalTransaction = await this.findTransactionById(captureData.transactionId);
          
          if (originalTransaction.type !== TransactionType.AUTHORIZATION) {
            throw new BadRequestException('Can only capture authorization transactions');
          }

          if (originalTransaction.status !== TransactionStatus.COMPLETED) {
            throw new BadRequestException('Cannot capture transaction that is not completed');
          }
          ...
          const savedCaptureTransaction = await queryRunner.manager.save(captureTransaction);
          ...
          savedCaptureTransaction.status = result.success ? TransactionStatus.COMPLETED : TransactionStatus.FAILED;
          ...
          this.logger.log(`Capture transaction ${savedCaptureTransaction.id} ${result.success ? 'completed' : 'failed'}`);
  ...
  async refundPayment(refundData: RefundPaymentDto): Promise<PaymentResponse> {
    return this.executeWithIdempotency(
      ...
      async () => {
        ...
        try {
          const originalTransaction = await this.findTransactionById(refundData.transactionId);
          ...
          const availableRefundAmount = originalTransaction.amount - originalTransaction.refundedAmount;
          ...
          const lastFourDigits = this.extractLastFourDigits(originalTransaction.gatewayResponse);
          ...
          savedRefundTransaction.status = result.success ? TransactionStatus.COMPLETED : TransactionStatus.FAILED;
          ...
          this.logger.log(`Refund transaction ${savedRefundTransaction.id} ${result.success ? 'completed' : 'failed'}`);
  ...
  async cancelPayment(cancelData: CancelPaymentDto): Promise<PaymentResponse> {
    return this.executeWithIdempotency(
      ...
      async () => {
        ...
        try {
          const originalTransaction = await this.findTransactionById(cancelData.transactionId);
          ...
          const savedVoidTransaction = await queryRunner.manager.save(voidTransaction);

          const result = await this.authorizeNetService.voidTransaction(cancelData);

          savedVoidTransaction.status = result.success ? TransactionStatus.COMPLETED : TransactionStatus.FAILED;
          ...
          this.logger.log(`Void transaction ${savedVoidTransaction.id} ${result.success ? 'completed' : 'failed'}`);
  ...
  private canRefundTransaction(transaction: Transaction): boolean {
    return [TransactionStatus.COMPLETED, TransactionStatus.PARTIALLY_REFUNDED].includes(transaction.status) &&
           [TransactionType.PURCHASE, TransactionType.CAPTURE].includes(transaction.type);
  }
  ...
  private mapToPaymentResponse(transaction: Transaction, result: PaymentResult): PaymentResponse {
    return {
